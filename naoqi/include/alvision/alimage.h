...
public:
  ...
  /**deepcopy
    ...
    *@note Returns an ALImage that has internal data, whether the object has
           internal or external data. Allocates a new data buffer and performs a
           copy of the data in the object.
     */
  ...
  /**
     ...
     *@note makes the object's data external
     */
...
private:
  /**
     *shared_ptr deleter class that tracks whether an fData buffer is internal.
     *
     *External deleters are inert and make their shared_ptr effectively akin to
     *a raw pointer. Internal deleters make their shared_ptr effectively akin to
     *a typical shared_ptr.
     *
     *ALImage uses its fData buffer's deleter to store the information about its
     *externality.
     *
     *It is safe to construct an uninitialized shared_ptr (with a NULL
     *underlying raw pointer) with a deleter of this type, in which case the
     *deleter will be inert, independently of its externality.
    *
     *In the case of an empty external buffer however, ALImage can't store its
     *externality in the deleter, as the deleter of a NULL pointer is undefined,
     *but it can define a NULL fData as being external; the case of an empty
     *fData on an internal ALImage being implemented as a new-allocated,
     *zero-sized non-NULL buffer with a non-inert deleter.
     */
  class deleter
  {
  public:
  ...
  /**
     *Returns the deleter associated with an internal or non-empty ALImage's fData.
     *
     *Only used in areDataExternal to access the externality of an ALImage.
     *Should not be called with on an empty, external ALImage. Such an ALImage
     *stores the fact it is external by testing fData for NULL.
     */

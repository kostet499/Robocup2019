...
namespace AL
{
   ...
   * \ingroup Proxy
   */
  class ALCOMMON_API ALProxy : public ::boost::enable_shared_from_this<ALProxy>
  {
  public:
    /**
     * \typedef Ptr
    ...
    /**
     * \brief Construction out of a session.
     * \param session A connected session. Throws otherwise.
    ...
    // This is movable
    // TODO VS2015: use default implementation
     ...
     * \return a cancelable future that will call stop(id) upon cancel
     */
    ...
    */
    ...
    std::unique_ptr<ALProxyPrivate> _p;
  ...
  /// Create a specialized proxy directly from a session.
  /// @param serviceName the name of the actual service to make a proxy to.
  /// @param session a connected session, otherwise would throw.
  /// @throw if the service could not be found.
  /// @note it does not throw if the proxy type mismatches the actual interface of the remote service
  template<typename ProxyType>
  boost::shared_ptr<ProxyType> makeProxy(
      const qi::SessionPtr& session,
      const std::string& serviceName)
  {
    return boost::make_shared<ProxyType>(boost::make_shared<ALProxy>(session, serviceName));
  }
}   // namespace AL

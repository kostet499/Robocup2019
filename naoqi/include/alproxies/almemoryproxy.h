...
namespace AL
{
  ...
  public:
    ...
    /// <summary>
    /// Gets a pointer to 32 a bit data item. Beware, the pointer will only
    /// be valid during the lifetime of the ALMemory object. Use with care, at initialization, not every loop.
    /// Insert a data item if needed. Throw if the data item has not the expected type.
    /// Only meaningful when called from code running in the same process as ALMemory.
    /// </summary>
    /// <param name="key"> Name of the data. </param>
    /// <returns> A pointer converted to void </returns>
    void* getDataPtr(const std::string& key);

    /// <summary>
    /// Gets a pointer to a int data item. Beware, the pointer will only
    /// be valid during the lifetime of the ALMemory object. Use with care, at initialization, not every loop.
    /// Insert a data item if needed. Throw if the data item has not the expected type.
    /// Only meaningful when called from code running in the same process as ALMemory.
    /// </summary>
    /// <param name="key"> Name of the data. </param>
    /// <returns> A pointer to int </returns>
    int* getIntPtr(const std::string& key);

    /// <summary>
    /// Gets a pointer to a float data item. Beware, the pointer will only
    /// be valid during the lifetime of the ALMemory object. Use with care, at initialization, not every loop.
    /// Insert a data item if needed. Throw if the data item has not the expected type.
    /// Only meaningful when called from code running in the same process as ALMemory.
    /// </summary>
    /// <param name="key"> Name of the data. </param>
    /// <returns> A pointer to float </returns>
    float* getFloatPtr(const std::string& key);

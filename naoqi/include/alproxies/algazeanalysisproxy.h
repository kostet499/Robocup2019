// Generated for ALGazeAnalysis version 0

#ifndef ALGAZEANALYSISPROXY_H_
#define ALGAZEANALYSISPROXY_H_
...
namespace AL
{
  ...
  namespace detail {
    class ALGazeAnalysisProxyPostHandler
    {
    protected:
      ALGazeAnalysisProxyPostHandler(boost::shared_ptr<ALProxy> proxy);

    public:
      friend class AL::ALGazeAnalysisProxy;
      ...
      /// <returns> brokerTaskID : The ID of the task assigned to it by the broker.</returns>
      int setFaceAnalysisEnabled(const bool& status);
      ...
      /// </summary>
      /// <param name="tolerance"> New value of the tolerance (between 0 and 1). </param>
      /// <returns> brokerTaskID : The ID of the task assigned to it by the broker.</returns>
      int setTolerance(const float& tolerance);
  ...
  /// <summary></summary>
  /// \ingroup ALProxies
  class ALPROXIES_API ALGazeAnalysisProxy
  {
  private:
    boost::shared_ptr<ALProxy> _proxy;
  ...
  public:
    ...
    ALGazeAnalysisProxy(boost::shared_ptr<ALBroker> broker);

    /// <summary>
    /// Explicit Proxy Constructor. Create a specialised proxy from a generic proxy.
    /// </summary>
    /// <param name="broker">A smart pointer to your broker</param>
    ALGazeAnalysisProxy(boost::shared_ptr<ALProxy> proxy);
    ...
    ALGazeAnalysisProxy(const std::string &ip, int port=9559);
    ...
    /// </summary>
    /// <param name="status"> True to turn it on, False to turn it off. </param>
    void setFaceAnalysisEnabled(const bool& status);
    ...
    detail::ALGazeAnalysisProxyPostHandler post;
  };

}
#endif // ALGAZEANALYSISPROXY_H_

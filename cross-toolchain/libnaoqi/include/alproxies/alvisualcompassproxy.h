// Generated for ALVisualCompass version 0
...
namespace AL
{
  ...
  namespace detail {
    class ALVisualCompassProxyPostHandler
    {
    protected:
      ALVisualCompassProxyPostHandler(boost::shared_ptr<ALProxy> proxy);

    public:
      friend class AL::ALVisualCompassProxy;
  ...
  public:
    ...
    ALVisualCompassProxy(const std::string &ip, int port=9559);
    ...
    /// </summary>
    /// <returns> [0]: Percentage of the matched keypoints that are used to compute the deviation (significant if over 50%)  </returns>
    ...
    /// </summary>
    /// <param name="x"> Algebric distance along the X axis in meters. </param>
    bool moveStraightTo(const float& x);
    ...
    bool setCurrentImageAsReference();
    ...
    detail::ALVisualCompassProxyPostHandler post;
  };

// Generated for ALSonar version 0

#ifndef ALSONARPROXY_H_
#define ALSONARPROXY_H_
...
namespace AL
{
  ...
  namespace detail {
    class ALSonarProxyPostHandler
    {
    protected:
      ALSonarProxyPostHandler(boost::shared_ptr<ALProxy> proxy);

    public:
      friend class AL::ALSonarProxy;
  ...
  /// <summary></summary>
  /// \ingroup ALProxies
  class ALPROXIES_API ALSonarProxy
  {
  private:
    boost::shared_ptr<ALProxy> _proxy;
  ...
  public:
    ...
    ALSonarProxy(boost::shared_ptr<ALBroker> broker);

    /// <summary>
    /// Explicit Proxy Constructor. Create a specialised proxy from a generic proxy.
    /// </summary>
    /// <param name="broker">A smart pointer to your broker</param>
    ALSonarProxy(boost::shared_ptr<ALProxy> proxy);
    ...
    ALSonarProxy(const std::string &ip, int port=9559);
    ...
    /// <summary>
    /// Returns a vector with rightand left filtered sonar values (in this order). Theses valuesare the distances to the nearest detected obstacle.
    /// </summary>
    std::vector<float> getFilteredValues();
    ...
    detail::ALSonarProxyPostHandler post;
  };

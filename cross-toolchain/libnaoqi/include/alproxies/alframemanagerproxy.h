...
namespace AL
{
  ...
  public:
    ...
    /// Creates a new behavior, from a box found in an xml file stored in the robot.
    /// </summary>
    /// <param name="packageDir"> Path to package directory, ex : "/path/to/appsName" </param>
    /// <param name="behaviorPath"> Relative path to the behavior inside the package ex : "behavior_1/behavior.xar" </param>
    /// <param name="behName">  </param>
    /// <returns> return a unique identifier for the created box, that can be used by playBehavior </returns>
    std::string createBehavior(const std::string& packageDir, const std::string &behaviorPath, const std::string& behName);

...
# define ALVALUE_VERBOSITY
  ...
  public:
     ...
     *
     * Assert the object is compatible with a wanted type.
     * If the object is not initialized, it will be changed to the good type
     * \throw ALERROR
     */
    ...
    /**
     * \brief Get pointer on int value.
     *        If the value is invalid, initialize it to 0. If the value is not
     *        invalid nor an int, throw.
     * \return A pointer on int value.
     */
    int *getIntPtr();

    /**
     * \brief Get pointer on float value.
     *        If the value is invalid, initialize it to 0.f. If the value is not
     *        invalid nor a float, throw.
     * \return A pointer on float value.
     */
    float *getFloatPtr();

...
export PATH="`/home/opennao/jenkins/workspace/yocto-rootfs/build/tmp/sysroots/x86_64-linux/usr/bin/python-native/python -c \"import os; print(os.pathsep.join([x for x in \\\"$PATH\\\".split(os.pathsep) if not any([d for d in ['cturtle', 'diamondback', 'electric', 'fuerte'] if d in x])]))\"`"
...
export ROS_PACKAGE_PATH="`/home/opennao/jenkins/workspace/yocto-rootfs/build/tmp/sysroots/x86_64-linux/usr/bin/python-native/python -c \"$PYTHON_CODE_BUILD_ROS_PACKAGE_PATH\"`"

# env variables in installspace
if [ -z "$CATKIN_ENV_HOOK_WORKSPACE" ]; then
  CATKIN_ENV_HOOK_WORKSPACE="/opt/ros/indigo"
fi

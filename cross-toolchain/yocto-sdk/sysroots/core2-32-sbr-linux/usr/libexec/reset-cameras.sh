...
GREP=/bin/grep
UNICORN_CAMERA_PCI_DEVID=10ee:0007
...
reset_camera_release() {
  ${I2CSET} -y "I2C primary cgos" 0x41 0x1 0xc  i
}

reset_camera_hold() {
  ${I2CSET} -y "I2C primary cgos" 0x41 0x1 0x0  i
}

reset_camera_config_output() {
  local cfg=$(${I2CGET} -y "I2C primary cgos" 0x41 0x3)
  ...
  ${I2CSET} -y "I2C primary cgos" 0x41 0x3 0xf3 i
}

if ${LSPCI} -n | ${GREP} -q ${UNICORN_CAMERA_PCI_DEVID} ; then
  # Pepper <1.8 HW not using the same camera HW
  exit 0
fi

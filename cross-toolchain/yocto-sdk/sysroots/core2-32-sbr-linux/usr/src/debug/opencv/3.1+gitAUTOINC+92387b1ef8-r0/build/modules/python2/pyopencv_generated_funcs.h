...
static PyObject* pyopencv_cv_face_face_MinDistancePredictCollector_MinDistancePredictCollector(PyObject* , PyObject* args, PyObject* kw)
{
    using namespace cv::face;

    double threshhold=DBL_MAX;
    pyopencv_face_MinDistancePredictCollector_t* self = 0;

    const char* keywords[] = { "threshhold", NULL };
    if( PyArg_ParseTupleAndKeywords(args, kw, "|d:MinDistancePredictCollector", (char**)keywords, &threshhold) )
    {
        self = PyObject_NEW(pyopencv_face_MinDistancePredictCollector_t, &pyopencv_face_MinDistancePredictCollector_Type);
        new (&(self->v)) Ptr<cv::face::MinDistancePredictCollector>(); // init Ptr with placement new
        if(self) ERRWRAP2(self->v.reset(new cv::face::MinDistancePredictCollector(threshhold)));
        return (PyObject*)self;
    }
...
static PyObject* pyopencv_cv_face_MinDistancePredictCollector_create(PyObject* , PyObject* args, PyObject* kw)
{
    using namespace cv::face;

    double threshold=DBL_MAX;
    Ptr<MinDistancePredictCollector> retval;

    const char* keywords[] = { "threshold", NULL };
    if( PyArg_ParseTupleAndKeywords(args, kw, "|d:MinDistancePredictCollector_create", (char**)keywords, &threshold) )
    {
        ERRWRAP2(retval = cv::face::MinDistancePredictCollector::create(threshold));
        return pyopencv_from(retval);
    }
...
static PyObject* pyopencv_cv_xphoto_autowbGrayworld(PyObject* , PyObject* args, PyObject* kw)
{
    ...
    const char* keywords[] = { "src", "dst", "thresh", NULL };
    if( PyArg_ParseTupleAndKeywords(args, kw, "O|Of:autowbGrayworld", (char**)keywords, &pyobj_src, &pyobj_dst, &thresh) &&
        pyopencv_to(pyobj_src, src, ArgInfo("src", 0)) &&
        pyopencv_to(pyobj_dst, dst, ArgInfo("dst", 1)) )
    {
        ERRWRAP2(cv::xphoto::autowbGrayworld(src, dst, thresh));
        return pyopencv_from(dst);
    }
...
static PyObject* pyopencv_cv_xphoto_balanceWhite(PyObject* , PyObject* args, PyObject* kw)
{
    using namespace cv::xphoto;
    ...
    const char* keywords[] = { "src", "dst", "algorithmType", "inputMin", "inputMax", "outputMin", "outputMax", NULL };
    if( PyArg_ParseTupleAndKeywords(args, kw, "OOi|ffff:balanceWhite", (char**)keywords, &pyobj_src, &pyobj_dst, &algorithmType, &inputMin, &inputMax, &outputMin, &outputMax) &&
        pyopencv_to(pyobj_src, src, ArgInfo("src", 0)) &&
        pyopencv_to(pyobj_dst, dst, ArgInfo("dst", 0)) )
    {
        ERRWRAP2(cv::xphoto::balanceWhite(src, dst, algorithmType, inputMin, inputMax, outputMin, outputMax));
        Py_RETURN_NONE;
    }

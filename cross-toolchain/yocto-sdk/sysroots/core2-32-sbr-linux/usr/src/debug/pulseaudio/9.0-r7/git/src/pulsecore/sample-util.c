...
static void calc_sine(float *f, size_t l, double freq, float amplitude) {
    size_t i;

    l /= sizeof(float);

    for (i = 0; i < l; i++)
        *(f++) = amplitude * sin((double) i*M_PI*2*freq / (double) l);
}

void pa_memchunk_sine(pa_memchunk *c, pa_mempool *pool, unsigned rate, unsigned freq, float amplitude) {
    size_t l;
    unsigned gcd, n;
    void *p;
    ...
    p = pa_memblock_acquire(c->memblock);
    calc_sine(p, c->length, freq * l / rate, amplitude);
    pa_memblock_release(c->memblock);
}

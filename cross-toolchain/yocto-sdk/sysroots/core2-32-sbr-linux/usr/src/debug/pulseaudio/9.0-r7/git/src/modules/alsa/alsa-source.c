...
static int unix_read(struct userdata *u, pa_usec_t *sleep_usec, bool polled, bool on_timeout) {
    ...
    /*
     * With alsa multi plugin we only read 1024 bytes block.
     */
    int min_block = 1024;
    ...
    for (;;) {
        ...
        n_bytes = (size_t) n * u->frame_size;
	if (n_bytes < min_block)
		return 0;
	n_bytes = n_bytes - (n_bytes % min_block);
...
pa_source *pa_alsa_source_new(pa_module *m, pa_modargs *ma, const char*driver, pa_card *card, pa_alsa_mapping *mapping) {
    ...
    /* FIXME: I hardcode these value because it's seems to be the only one which works
     * with our wonderful Poulsbo chipset, and our version of snd_hda_intel (2.6.32).
     * Sorry for this one.
     */
    nfrags = 4;
    frag_size = 16384;
    ...
    /* FIXME: mmap and tsched doesn't work too for input,
     * but is ok for output, so let hardcode the parameter here.
     * The good solution is to have a working driver.
     */
    use_tsched = false;
    use_mmap   = false;

...
int pa__init(pa_module*m) {
    ...
    if (pa_modargs_get_value_u32(ma, "frequency", &frequency) < 0 || frequency < 1 || frequency > ss.rate/2) {
        pa_log_error("Invalid frequency specification 1 < rate/1=%d < freq=%d", ss.rate/2, frequency);
        goto fail;
    }

    amplitude = 0.5;
    if (pa_modargs_get_value_double(ma, "amplitude", &amplitude) < 0 || amplitude < 0.0 || amplitude > 1.0) {
        pa_log_error("Invalid amplitude specification 0.0 < %lf < 1.0", amplitude);
        goto fail;
    }
    ...
    u->peek_index = 0;
    pa_memchunk_sine(&u->memchunk, m->core->mempool, ss.rate, frequency, amplitude);
    ...
    pa_proplist_setf(data.proplist, "sine.amp", "%lf", amplitude);

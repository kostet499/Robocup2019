...
static pa_hook_result_t sink_fixate_hook_callback(pa_core *c, pa_sink_new_data *new_data, struct userdata *u) {
    ...
    if ((e = perportentry_read(u, name, new_data->active_port))) {
        ...
        pa_log_info("Initial volume (restoring) for sink %s", new_data->name);
        if (!new_data->volume_is_set) {
            pa_cvolume v;
            pa_channel_map map;
            char buf[PA_CVOLUME_SNPRINT_VERBOSE_MAX];
            /* Set basic 50% volume (magic number) */
            v.channels = 1;
            v.values[0] = PA_VOLUME_NORM*50/100;
            map.channels = 1;
            map.map[0] = PA_CHANNEL_POSITION_MONO;

            pa_cvolume_remap(&v, &map, &new_data->channel_map);
            pa_sink_new_data_set_volume(new_data, &v);
            pa_log_info("Set Initial volume for sink %s: %s", new_data->name,
                        pa_cvolume_snprint_verbose(buf, sizeof(buf), &new_data->volume, &new_data->channel_map, false));

            new_data->save_volume = true;
        } else
            pa_log_debug("Not set initial volume for sink %s, because already configured.", new_data->name);
    }

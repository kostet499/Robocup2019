...
enum {
    ...
    SINK_INPUT_MESSAGE_REQUEST_CORK,
    SINK_INPUT_MESSAGE_REQUEST_UNCORK
};
...
static void source_output_push_cb(pa_source_output *o, const pa_memchunk *chunk) {
    ...
    pa_mutex_lock(u->mx);
    if (u->corked == true) {
      /* So there is data coming back, we have to uncork the stream,
       * but we need first to go to the main thread to do that.
       */
      pa_sink_input_ref(u->sink_input);
      pa_asyncmsgq_post(pa_thread_mq_get()->outq, PA_MSGOBJECT(u->sink_input), SINK_INPUT_MESSAGE_REQUEST_UNCORK, NULL, 0, NULL, NULL);
      u->corked = false;
    }
    pa_mutex_unlock(u->mx);
...
static int source_output_process_msg_cb(pa_msgobject *obj, int code, void *data, int64_t offset, pa_memchunk *chunk) {
    ...
    switch (code) {
        ...
        case SOURCE_OUTPUT_MESSAGE_LATENCY_SNAPSHOT: {
            ...
            /* Fix #35850 The latency increase when no data is provide in the source ==> that force the reduce of the latency (bad case) */
...
static int sink_input_pop_cb(pa_sink_input *i, size_t nbytes, pa_memchunk *chunk) {
    ...
    if (pa_memblockq_peek(u->memblockq, chunk) < 0) {
        ...
        pa_mutex_lock(u->mx);
        u->idle_cnt++;
        if (u->idle_cnt > 5 && u->corked == false) {
          pa_log_info("Corking the sink-input");
          pa_sink_input_ref(u->sink_input);
            pa_asyncmsgq_post(pa_thread_mq_get()->outq, PA_MSGOBJECT(u->sink_input), SINK_INPUT_MESSAGE_REQUEST_CORK, NULL, 0, NULL, NULL);
            u->corked = true;
            u->idle_cnt = 0;
        }
        pa_mutex_unlock(u->mx);
...
static int sink_input_process_msg_cb(pa_msgobject *obj, int code, void *data, int64_t offset, pa_memchunk *chunk) {
    ...
    switch (code) {
        ...
        case SINK_INPUT_MESSAGE_REQUEST_CORK: {
            /* This message is sent from the IO thread to the main
             * thread! So don't be confused. All the use cases above
             * are executed in thread context, but this one is not! */

            pa_assert_ctl_context();
            pa_log_info("%s: corking",__func__);
        ...
        case SINK_INPUT_MESSAGE_REQUEST_UNCORK: {
            /* This message is sent from the IO thread to the main
             * thread! So don't be confused. All the use cases above
             * are executed in thread context, but this one is not! */

            pa_assert_ctl_context();
            pa_log_info("%s: uncorking",__func__);
